...
if(isset($_POST['insert'])){
    ...
    if (isset($_POST['asset_create_remark'])) {
        $create_remark = implode(', ', $_POST['asset_create_remark']);
        echo $create_remark ;
      }else{
        $create_remark = null;
      }
    ...
    if($image_file != null){
        ...
        if($type =="image/jpg" || $type == "image/jpeg" || $type == "image/png"){
            ...
            if($size < 2000000){
                ...
                date_buy,seller,id_quotation,id_po,create_remark,create_by,working) 
...
// ============== broken ==============
if(isset($_POST['broken_asset'])){

    $id = $_POST['broken_id'];
    echo $id;
    $broken = $pdo->prepare("UPDATE asset SET working='bad',broken_date=NOW() WHERE id = '$id'");
    if($broken->execute()){
        $_SESSION['broken_status'] = 'broken_success';
        header('location:Asset.php');   
          
      }else{
        $_SESSION['broken_status'] = 'broken_fail';
        header('location:Asset.php');   
      }
}

// ============== broken Return==============
if(isset($_POST['broken_return_asset'])){

    $id = $_POST['broken_id'];
    echo $id;
    $broken = $pdo->prepare("UPDATE asset SET working='good', broken_return_date=NOW() WHERE id = '$id'");
    if($broken->execute()){
        $_SESSION['broken_status'] = 'broken_success';
        header('location:Asset.php');   
