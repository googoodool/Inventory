...
if(isset($_POST['insert'])){
    ...
    if(isset($_POST['pro_name'])){
        $select = $pdo->prepare("SELECT station_name FROM station WHERE station_name='$pro_name'");
        $select->execute();
        ...
        if($select->rowCount() > 0){
          ...
          header('location:Register_project.php');   
            ...
            $insert = $pdo->prepare("INSERT INTO station (station_name,create_by)
...
if(isset($_POST['delete_project'])){
    ...
    $id = $_POST['delete_id'];
    $delete = $pdo->prepare("DELETE FROM station WHERE id = '$id'");
    if($delete->execute()){
